...
Для тестирования с интервалом 5 минут
"""
...
class RealTelegramSender:
    """Реальный отправитель в Telegram"""
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as response:
                ...
                if result.get('ok'):
                    ...
                    print(f"✅ Сообщение #{self.messages_sent} отправлено в Telegram!")
                    print(f"   ID: {result['result']['message_id']}")
                    return True
                else:
...
class RealTestAutomation:
    def __init__(self):
        ...
        print(f"📱 Telegram канал: {self.config['telegram_channel_id']}")
        print(f"🔑 Токен: {self.config['telegram_token'][:10]}...{self.config['telegram_token'][-10:]}")
        ...
        try:
            ...
            if best_analyses:
                ...
                for analysis in best_analyses:
                    ...
                    if success:
                        print(f"✅ Прогноз отправлен: {analysis.match_id}")
                    else:
                        print(f"❌ Ошибка отправки: {analysis.match_id}")
...
🚀 **TrueLiveBet система всегда на вашей стороне!** ⚽📱
    ...
    async def start(self):
        ...
        print("🚀 Запуск реальной автоматизации TrueLiveBet...")
        ...
        try:
            while self.is_running:
                ...
                if self.is_running:
                    ...
                    print(f"🕐 Следующий цикл в: {next_time_str}")
